...
const memberMoreLinks = [
  ...
  { to: '/excusals', label: 'Excusals', description: 'Submit and track your excusal requests.' },
...
const adminMoreLinks = [
  ...
  { to: '/admin/members', label: 'Member Management', description: 'View approved members.' },
]

const standardsMoreLinks = [
  { to: '/admin/standards', label: 'Standards Dashboard', description: 'Attendance risk and excusal review.' },
]

const analyticsMoreLinks = [
  { to: '/admin/analytics', label: 'Analytics', description: 'Review engagement trends.' },
]

function More() {
  const { currentUser } = useAuth()
  const adminLinks = isAdminRole(currentUser)
    ? [
        ...adminMoreLinks.slice(0, 1),
        ...(canAccessStandards(currentUser) ? standardsMoreLinks : []),
        ...adminMoreLinks.slice(1),
        ...(canViewAnalytics(currentUser) ? analyticsMoreLinks : []),
      ]
    ...
    : []
  const links = [...memberMoreLinks, ...adminLinks]
